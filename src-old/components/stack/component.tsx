...
export const Stack: FC<StackProps> = (props) => {
  ...
  const { className, style, Component } = useMemo(() => {
    return {
      ...
      style: setStyles([styleLayout, props.style]),
...
export const StackMotion: FC<StackMotionProps> = (props) => {
  ...
  const { className: className2, style:style2, as } = useMemo(() => {
    return {
      className: setClasses([
        styles.stack,
      ...
      style: setStyles([styleLayout, style]),
      as: props.as ?? 'div',
    };
  }, [props.as, styleLayout, isHover, isRipple, className, style]);
